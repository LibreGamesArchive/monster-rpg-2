...
extern MFONT *big_font;


class WgtFrame : public TGUIWidget {
public:
	void draw(void);
	int update(int millis);
...
class WgtButton : public TGUIWidget {
public:
	void draw();
	int update(int millis);
	void mouseDown(int mx, int my, int mb);
	void mouseUp(int x, int y, int b);
	bool handleKey(int keycode, int unichar);
	bool acceptsFocus();
	WgtButton(int x, int y, int width, int height,
			std::string text, MCOLOR text_color,
			MCOLOR button_color, MFONT *font);
	virtual ~WgtButton();
protected:
	std::string text;
	MCOLOR text_color;
...
class WgtScrollbar : public TGUIWidget {
public:
	static const int TAB_MIN_SIZE = 18;
	void draw();
	int update(int millis);
	void mouseDown(int mx, int my, int mb);
	void mouseUp(int x, int y, int b);
	float getValue(void);
	void setParams(float start, float end, float step);
	void setValue(float value);
	WgtScrollbar(bool vertical, int x, int y, int width, int height,
			float start, float end, float extent,
			MCOLOR tab_color, MCOLOR trough_color);
	virtual ~WgtScrollbar();
protected:
	int calcTabSize(void);
	MCOLOR tab_color;
	MCOLOR trough_color;
	bool pressed; // mouseDown happened
	int pressedPos;
	bool vertical;
	int tab_size;
	float start, end, extent;
	int tabPos;
};


class WgtMenuItem : public TGUIWidget {
public:
	void draw();
	int update(int millis);
	void mouseDown(int mx, int my, int mb);
	...
	void show(void);
	bool handleKey(int keycode, int unichar);
	WgtMenuItem(int x, int y, int width, int height,
...
private:
	std::string name;
	std::vector<WgtMenuItem *> items;
};


class WgtMenuBar : public TGUIWidget {
public:
	void draw();
	int update(int millis);
	void mouseDown(int mx, int my, int mb);
	void hideMenus(void);
	bool handleHotkey(int hotkey);
	WgtMenuBar(int x, int y, int width, int height,
...
class WgtRadioButton : public TGUIWidget {
public:
	void draw();
	int update(int millis);
	void mouseDown(int mx, int my, int mb);
	...
	void setNext(WgtRadioButton *next);
	bool isChecked(void);
	WgtRadioButton *getNext(void);
	WgtRadioButton *getPrev(void);
	WgtRadioButton(int x, int y, std::string text,
			MCOLOR textColor,
			MCOLOR highlightColor, MFONT *font,
			WgtRadioButton *previous);
	virtual ~WgtRadioButton();
protected:
	std::string text;
	MCOLOR textColor;
	...
	MFONT *font;
	bool checked;
	WgtRadioButton *next, *prev;
};


class WgtLabel : public TGUIWidget {
public:
	void draw();
	int update(int millis);
	WgtLabel(int x, int y, std::string text,
			MCOLOR textColor,
			MCOLOR shadowColor,
			MFONT *font);
	virtual ~WgtLabel();
...
class WgtTextField : public TGUIWidget {
public:
	void draw();
	int update(int millis);
	bool handleKey(int keycode, int unichar);
	...
	WgtTextField(int x, int y, int width, int height, int maxChars,
			std::string text,
			std::string filterChars,
			MCOLOR textColor,
			MCOLOR bgColor, MFONT *font);
	virtual ~WgtTextField();
protected:
	bool keyPassesFilter(int key);
	char *buffer;
	std::string text;
	std::string filterChars;
	MCOLOR textColor;
...
class WgtAreaEditor : public TGUIWidget {
public:
	void draw();
	int update(int millis);
	void mouseDown(int mx, int my, int mb);
	void mouseUp(int x, int y, int b);
	WgtAreaEditor(int x, int y, int width, int height);
	virtual ~WgtAreaEditor();
protected:
	bool brushStarted;
	int lastSolidX;
	int lastSolidY;
...
class WgtTileSelector : public TGUIWidget {
public:
	void draw();
	int update(int millis);
	void mouseUp(int mx, int my, int mb);
	WgtTileSelector(int x, int y, int width, int height,
		int scale, WgtScrollbar *vbar, WgtScrollbar *hbar);
	virtual ~WgtTileSelector();
private:
	int scale;
	WgtScrollbar *vbar, *hbar;
	int rows, cols;
};

class WgtCrappyWidget : public TGUIWidget {
public:
	void draw();
	int update(int millis);
	bool handleKey(int keycode, int unichar);
	...
	void setText(std::string newText);
	std::string getText(void);
	WgtCrappyWidget(int x, int y, int width, int height, int maxChars,
			std::string text,
			std::string filterChars,
			MCOLOR textColor,
			MCOLOR bgColor, MFONT *font);
	virtual ~WgtCrappyWidget();
protected:
	bool keyPassesFilter(int key);
	char *buffer;
	std::string text;
