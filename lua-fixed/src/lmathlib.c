...
#define RAD_TO_FIXANGLE(r) (fixmul(r, 0x28be61))

typedef int fixed;

static fixed _lua_cos_tbl[512] =
{
...
static fixed _lua_acos_tbl[513] =
{
...
#if 0
static unsigned short _sqrt_table[256] =
{
...
static inline fixed fixfloor(fixed x)
{
   return (x & 0xFFFF0000);
}
...
{
   if (x & 0xFFFF)
      return fixadd(x & 0xFFFF0000, 0x00010000);
   else
      return x & 0xFFFF0000;
}
...
static inline int fixtoi(fixed x)
{
   ...
   return (x >> 16) | ((x & 0x80000000) ? 0xFFFF0000 : 0);
}


static inline fixed fixcos(fixed x)
{
   return _lua_cos_tbl[((x + 0x4000) >> 15) & 0x1FF];
}


static inline fixed fixsin(fixed x)
{
   return _lua_cos_tbl[((x - 0x400000 + 0x4000) >> 15) & 0x1FF];
}


static inline fixed fixtan(fixed x)
{
   return _lua_tan_tbl[((x + 0x4000) >> 15) & 0xFF];
}


static inline fixed fixacos(fixed x)
{
   return _lua_acos_tbl[(x+65536+127)>>8];
}


static inline fixed fixasin(fixed x)
{
   return 0x00400000 - _lua_acos_tbl[(x+65536+127)>>8];
}

static inline fixed fixatan(fixed x)
{
...
static inline fixed fixatan2(fixed y, fixed x)
{
   fixed r;
...
{
   if (x > 0)
      return ftofix(sqrtf(fixtof(x)));

   return 0;
}


static inline fixed fixabs(fixed f)
{
	if (f & 0x80000000) {
		int i = (f & 0xFFFF0000) >> 16;
		return (((0xFFFF - i) + 1) << 16) | (f & 0xFFFF);
	}
	else {
		return f;
	}
...
static int math_fixadd(lua_State *L)
{
  lua_pushnumber(L, fixadd(luaL_checknumber(L, 1),
  	luaL_checknumber(L, 2)));
  return 1;
}


static int math_fixsub(lua_State *L)
{
  lua_pushnumber(L, fixsub(luaL_checknumber(L, 1),
  	luaL_checknumber(L, 2)));
  return 1;
}


static int math_fixmul(lua_State *L)
{
  lua_pushnumber(L, fixmul(luaL_checknumber(L, 1),
  	luaL_checknumber(L, 2)));
  return 1;
}


static int math_fixdiv(lua_State *L)
{
  lua_pushnumber(L, fixdiv(luaL_checknumber(L, 1),
  	luaL_checknumber(L, 2)));
  return 1;
}


static int math_frac2fix(lua_State *L) {
  lua_pushnumber(L, fixdiv(itofix(luaL_checknumber(L, 1)),
  	itofix(luaL_checknumber(L, 2))));
  return 1;
}

static int math_itofix (lua_State *L) {
  lua_pushnumber(L, itofix(luaL_checknumber(L, 1)));
  return 1;
}

static int math_fixtoi (lua_State *L) {
  lua_pushnumber(L, fixtoi(luaL_checknumber(L, 1)));
  return 1;
}

static int math_fixabs (lua_State *L) {
  lua_pushnumber(L, fixabs(luaL_checknumber(L, 1)));
  return 1;
}

static int math_fixsin (lua_State *L) {
  lua_pushnumber(L, fixsin(RAD_TO_FIXANGLE(luaL_checknumber(L, 1))));
  return 1;
}

static int math_fixsinh (lua_State *L) {
  lua_pushnumber(L, ftofix(sinhf(fixtof(luaL_checknumber(L, 1)))));
  return 1;
}

static int math_fixcos (lua_State *L) {
  lua_pushnumber(L, fixcos(RAD_TO_FIXANGLE(luaL_checknumber(L, 1))));
  return 1;
}

static int math_fixcosh (lua_State *L) {
  lua_pushnumber(L, ftofix(coshf(fixtof(luaL_checknumber(L, 1)))));
  return 1;
}

static int math_fixtan (lua_State *L) {
  lua_pushnumber(L, fixtan(RAD_TO_FIXANGLE(luaL_checknumber(L, 1))));
  return 1;
}

static int math_fixtanh (lua_State *L) {
  lua_pushnumber(L, ftofix(tanhf(fixtof(luaL_checknumber(L, 1)))));
  return 1;
}

static int math_fixasin (lua_State *L) {
  lua_pushnumber(L, fixasin(RAD_TO_FIXANGLE(luaL_checknumber(L, 1))));
  return 1;
}

static int math_fixacos (lua_State *L) {
  lua_pushnumber(L, fixacos(RAD_TO_FIXANGLE(luaL_checknumber(L, 1))));
  return 1;
}

static int math_fixatan (lua_State *L) {
  lua_pushnumber(L, fixatan(RAD_TO_FIXANGLE(luaL_checknumber(L, 1))));
  return 1;
}

static int math_fixatan2 (lua_State *L) {
  lua_pushnumber(L, fixatan2(luaL_checknumber(L, 1), luaL_checknumber(L, 2)));
  return 1;
}

static int math_fixceil (lua_State *L) {
  lua_pushnumber(L, fixceil(luaL_checknumber(L, 1)));
  return 1;
}

static int math_fixfloor (lua_State *L) {
  lua_pushnumber(L, fixfloor(luaL_checknumber(L, 1)));
  return 1;
}

static int math_fixfmod (lua_State *L) {
  lua_pushnumber(L, ftofix(fmodf(fixtof(luaL_checknumber(L, 1)), fixtof(luaL_checknumber(L, 2)))));
  return 1;
}

static int math_fixmodf (lua_State *L) {
  float ip;
  fixed fp = ftofix(modff(fixtof(luaL_checknumber(L, 1)), &ip));
  lua_pushnumber(L, ftofix(ip));
  lua_pushnumber(L, fp);
  return 2;
}

static int math_fixsqrt (lua_State *L) {
  lua_pushnumber(L, fixsqrt(luaL_checknumber(L, 1)));
  return 1;
}

static int math_fixpow (lua_State *L) {
  lua_pushnumber(L, ftofix(powf(fixtof(luaL_checknumber(L, 1)), fixtof(luaL_checknumber(L, 2)))));
  return 1;
}

static int math_fixlog (lua_State *L) {
  lua_pushnumber(L, ftofix(logf(fixtof(luaL_checknumber(L, 1)))));
  return 1;
}

static int math_fixlog10 (lua_State *L) {
  lua_pushnumber(L, ftofix(log10f(fixtof(luaL_checknumber(L, 1)))));
  return 1;
}

static int math_fixexp (lua_State *L) {
  lua_pushnumber(L, ftofix(expf(fixtof(luaL_checknumber(L, 1)))));
  return 1;
}

static int math_fixdeg (lua_State *L) {
  lua_pushnumber(L, fixdiv(luaL_checknumber(L, 1), ftofix(RADIANS_PER_DEGREE)));
  return 1;
}

static int math_fixrad (lua_State *L) {
  lua_pushnumber(L, fixmul(luaL_checknumber(L, 1), ftofix(RADIANS_PER_DEGREE)));
  return 1;
}

static int math_fixfrexp (lua_State *L) {
  int e;
  lua_pushnumber(L, ftofix(frexpf(fixtof(luaL_checknumber(L, 1)), &e)));
  lua_pushinteger(L, itofix(e));
  return 2;
}

static int math_fixldexp (lua_State *L) {
  lua_pushnumber(L, ftofix(ldexpf(fixtof(luaL_checknumber(L, 1)), fixtof(luaL_checkint(L, 2)))));
  return 1;
}



static int math_fixmin (lua_State *L) {
...
static int math_fixmax (lua_State *L) {
  int n = lua_gettop(L);  /* number of arguments */
...
static int math_fixrandom (lua_State *L) {
  /* the `%' avoids the (rare) case of r==1, and is needed also because on
     some systems (SunOS!) `rand()' may return a value larger than RAND_MAX */
  fixed r = ftofix((rand()%RAND_MAX) / (float)RAND_MAX);
  switch (lua_gettop(L)) {  /* check number of arguments */
    ...
    case 1: {  /* only upper limit */
      fixed u = luaL_checkint(L, 1);
      //luaL_argcheck(L, 1<=u, 1, "interval is empty");
      lua_pushnumber(L, fixfloor(fixadd(fixmul(r, u), 1)));  /* int between 1 and `u' */
      break;
    }
    case 2: {  /* lower and upper limits */
      fixed l = luaL_checkint(L, 1);
      fixed u = luaL_checkint(L, 2);
      //luaL_argcheck(L, l<=u, 2, "interval is empty");
      lua_pushnumber(L, fixfloor(fixadd(fixmul(r, (u-l+1)), l)));  /* int between `l' and `u' */
      break;
    }
...
static int math_fixrandomseed (lua_State *L) {
  srand(fixtoi(luaL_checkint(L, 1)));
  return 0;
}
...
static const luaL_Reg mathlib[] = {
  {"fixabs",   math_fixabs},
  {"fixacos",  math_fixacos},
  {"fixasin",  math_fixasin},
  {"fixatan2", math_fixatan2},
  {"fixatan",  math_fixatan},
  {"fixceil",  math_fixceil},
  {"fixcosh",   math_fixcosh},
  {"fixcos",   math_fixcos},
  {"fixdeg",   math_fixdeg},
  {"fixexp",   math_fixexp},
  {"fixfloor", math_fixfloor},
  {"fixfmod",   math_fixfmod},
  {"fixfrexp", math_fixfrexp},
  {"fixldexp", math_fixldexp},
  {"fixlog10", math_fixlog10},
  {"fixlog",   math_fixlog},
  {"fixmax",   math_fixmax},
  {"fixmin",   math_fixmin},
  {"fixmodf",   math_fixmodf},
  {"fixpow",   math_fixpow},
  {"fixrad",   math_fixrad},
  {"fixrandom",     math_fixrandom},
  {"fixrandomseed", math_fixrandomseed},
  {"fixsinh",   math_fixsinh},
  {"fixsin",   math_fixsin},
  {"fixsqrt",  math_fixsqrt},
  {"fixtanh",   math_fixtanh},
  {"fixtan",   math_fixtan},
  {"itofix", math_itofix},
  {"fixtoi", math_fixtoi},
  {"frac2fix", math_frac2fix},
  {"fixadd",   math_fixadd},
  {"fixsub",   math_fixsub},
  {"fixmul",   math_fixmul},
  {"fixdiv",   math_fixdiv},
...
LUALIB_API int luaopen_math (lua_State *L) {
  ...
  lua_pushnumber(L, ftofix(PI));
  lua_setfield(L, -2, "fixpi");
  //lua_pushnumber(L, HUGE_VAL);
  //lua_setfield(L, -2, "huge");
#if defined(LUA_COMPAT_MOD)
  lua_getfield(L, -1, "fixfmod");
  lua_setfield(L, -2, "fixmod");
#endif
  return 1;
}
