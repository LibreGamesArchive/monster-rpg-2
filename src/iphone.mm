...
static ALLEGRO_BITMAP *m_create_bitmap_no_init(int w, int h, int true_w, int true_h)
{
	pvr = true;
	pvr_true_w = true_w;
	pvr_true_h = true_h;
	ALLEGRO_BITMAP *b = m_create_bitmap(w, h);
	pvr = false;
	return b;
}
...
ALLEGRO_BITMAP *m_load_pvr_bitmap(const char *filename)
{
	bool found_info = false;
	int portion_w = -1, portion_h = -1;
	char buf[1000];
	sprintf(buf, "%s.info.gz", filename);
	gzFile f = gzopen(buf, "rb");
	if (f) {
		portion_w = igetl(f);
		portion_h = igetl(f);
		gzclose(f);
...
//    BOOL success = FALSE;
    ...
    int _internalFormat = GL_COMPRESSED_RGBA_PVRTC_4BPPV1_IMG;
    bool _hasAlpha;
	...
	if (found_info) {
		b = m_create_bitmap_no_init(portion_w, portion_h, width, height);
	}
	else {
		b = m_create_bitmap_no_init(width, height, width, height);
	}
